...
**Dex3 hands.** `scene_g1_dex3.xml` (generated by
[`scripts/make_g1_dex3_scene.py`](../scripts/make_g1_dex3_scene.py), for
[`deploy/robots/g1_dex3`](../deploy/robots/g1_dex3/README.md)) adds the two Dex3-1 hands
as 14 extra actuators after the 29 body motors. The bridge keeps the 29 on `rt/lowcmd` /
`rt/lowstate` and serves each hand on `rt/dex3/<side>/cmd` (`HandCmd_`, 7 motors) and
`rt/dex3/<side>/state` (`HandState_`), exactly like the real hands. The hand actuators are
MuJoCo `<position>` servos rather than `<motor>`s: the command's `q` goes to `ctrl` and its
`kp`/`kd` into the servo gain/bias parameters every bridge tick, because a bridge-side PD
on the fingers' ~3e-4 kg·m² is unstable when one command is held over the multi-step
physics bursts the simulator takes. For the same reason the scene uses the `implicitfast`
integrator (as mjlab does): under Euler the servo damping on the ~1e-4 kg·m² distal links
is explicit and chatters at the force limit. Hand/body contacts are disabled through
collision bitmasks, matching the training entity's exclusion. `dq`/`tau`/`mode` of the
command are ignored; fingers go limp 1 s after the last command.
